...
Note that files in `docs/solutions/` are intentional, permanent learning artifacts, and files in `docs/plans/` are working plans that the project deletes at version close. Do not recommend their removal or characterize them as unnecessary merely because they are generated by a workflow.
